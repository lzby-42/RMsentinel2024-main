...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
//void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan);//加入的代�????????2
/* USER CODE END 0 */
...
int main(void)
  ...
  imu_ahrs_update();
  imu_attitude_update();
  while (imu.temp < 40.0f)
  {
    mpu_get_data();
    imu_ahrs_update();
    imu_attitude_update();
    heat_gyro(2885.0f, 0.0f, 288.4f);
    // heat_gyro(1400.0f, 0.0f, 88.4f);
    HAL_Delay(1);
  }
  for (i = 1; i <= 1000; i++)
  {
